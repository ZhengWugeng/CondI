...
class Logger(logging.Logger):
    ...
    def log_once(self, *args, **kwargs):
        ...
        self.server.test_metric = test_metric
        ...
        self.info(f"--- Client Training Status ---")
        for met_name, met_val in train_metrics.items():
            if "_" in met_name: # cx_acc, cx_loss
                continue
            
            self.info(f"Client Train {met_name}: {met_val}")
        ...
        if valid_metrics is not None:
            self.info(f"--- Client Validation Status ---")
            for met_name, met_val in valid_metrics.items():
                if "_" in met_name: # cx_acc, cx_loss
                    continue
                
                self.info(f"Client Valid {met_name}: {met_val}")
