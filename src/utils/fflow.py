...
def read_option():
    ...
    # multimodal setting
    parser.add_argument('--contrastive_weight', help='Modality contrastive loss weight;', type=float, default=0.2)
    ...
    parser.add_argument('--fedmsplit_prox_lambda', help='lambda of proximity term', type=float, default=0.0)
    ...
    parser.add_argument("--test_ps", help="Test p_sample | Probability to erase modality of test samples", type=float, default=0.0)
    parser.add_argument("--test_pm", help="Test p_modality | Probability to erase modality of test samples", type=float, default=0.0)
    parser.add_argument("--adaptive", help="Adaptive forward or not", action="store_true", default=False)
    parser.add_argument("--graph_quantile", help='Graph removing rate by quantile', type=float, default=0.75)
    parser.add_argument("--top_k", type=int, default=3)
    
    # CondI diffusion imputation setting
    parser.add_argument("--num_diff_steps", help="Number of diffusion training steps per federated round", type=int, default=1)
    parser.add_argument("--diff_lr", help="Learning rate for diffusion module", type=float, default=None)
    parser.add_argument("--mask_ratio", help="Fraction of observed values masked as pseudo-targets", type=float, default=0.2)
    parser.add_argument("--sample_missing_ratio", help="Missing ratio inside each sample for dataset-level partial missing", type=float, default=0.5)
    parser.add_argument("--diffusion_timesteps", help="Number of diffusion steps T", type=int, default=50)

    # prompt pool local missing
    parser.add_argument("--num_outer_loops", help="Non-parametric Aggregation in probabilistic prompt", type=int, default=10)
    parser.add_argument("--num_fedavg_rounds", help="Number of FedAvg rounds before probabilistic prompt pool happens", type=int, default=0)
    
    # debugging
    parser.add_argument("--visualize", help="Visualize mode to analyze feature. Pretrained weight required", action='store_true')
    ...
    try:
        if option['pretrain'] != '':
            try:
                ...
                loaded_state_dict = torch.load(option['pretrain'], map_location=torch.device('cpu'))
                new_state_dict = {}
                print("Mismatch at: ")
