...
class Model(FModule):
    ...
    def forward(self, x, y, leads, quantile=0.75, visualize=False, matrix=None, contrastive_weight=5.0): # x: B x 12 x C
        ...
        for lead in total_lead_ind:    
            ...
            # start extraction
            ori_feature = feature = self.feature_extractors[lead](x[:, lead, :].view(batch_size, 1, -1))
            leads_features.append(feature)
            relation_info = self.relation_embedders[lead](y.device, has_modal=True).repeat(batch_size,1)
            added_feature = feature = feature + relation_info
            imputed_feature = self.relation_embedders[lead](y.device, has_modal=False).repeat(batch_size,1)        # self.hidden_dim, 256
            
            # collect feature
            features[:,lead*self.hidden_dim:(lead+1)*self.hidden_dim] = added_feature * (1-missing_mask) + missing_mask * imputed_feature
            imputed_feature = ori_feature * (1 - missing_mask) + missing_mask * imputed_feature
            feature_extractor_outputs += imputed_feature
            
        if visualize:
            self.stored_features['final_feature'] = features.detach().cpu()
