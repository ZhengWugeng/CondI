...
class LeadsMoEFuser(nn.Module):
    """
    Concat -> Experts MoE fuser for 12 leads
    """
    def __init__(
        self,
        num_leads: int = 12,
        ...
        in_dim = num_leads * lead_dim + num_leads # + missing flags
    ...
    def forward(self, leads_vec, leads_missing):
        # leads_vec: (B, K, D)
        # leads_missing: (B, K)  (1=missing, 0=present)
        B, K, D = leads_vec.shape
        
        # Zero-out missing modality embeddings
        miss_mask = leads_missing.unsqueeze(-1) # (B, K, 1)
        leads_vec = leads_vec * (1.0 - miss_mask)
        
        x = torch.cat([leads_vec.view(B, K*D), leads_missing], dim=-1)  # (B, in_dim)
...
class CSDI_base(FModule):
    ...
    def get_side_info_mm(self, observed_tp, cond_mask, fused_ctx, w_cond):
        base_side = self.get_side_info(observed_tp, cond_mask)  # (B, base, K, L)
        B, _, K, L = base_side.shape

        ctx = fused_ctx[:, :, None, None].expand(B, self.ctx_dim, K, L)

        # w_cond is modality-aware: (B, K, D) -> (B, D, K, L)
        if w_cond is None:
            cond = base_side.new_zeros((B, self.cond_dim, K, L))
        else:
            cond = w_cond.permute(0, 2, 1).unsqueeze(-1).expand(B, self.cond_dim, K, L)

        return torch.cat([base_side, ctx, cond], dim=1)

    def _sync_runtime_device(self, runtime_device):
        if self.alpha_torch.device != runtime_device:
            self.alpha_torch = self.alpha_torch.to(runtime_device)
    ...
    def forward(self, batch, is_train=1):
        ...
        self._sync_runtime_device(observed_data.device)

        leads_vec = batch.get("leads_vec", None)
        ...
        w_cond = batch.get("w_cond", None)

        if leads_vec is not None and hasattr(self, 'moe'):
            # Partial-missing setting: use per-lead missing ratio (0..1),
            # not only the all-missing lead indicator.
            leads_missing = 1.0 - cond_mask.mean(dim=-1)
            fused_ctx, _ = self.moe(leads_vec, leads_missing)
            side_info = self.get_side_info_mm(observed_tp, cond_mask, fused_ctx, w_cond)
        else:
    ...
    def impute_mm(self, batch, n_samples):
        (
            observed_data,
            observed_mask,
            observed_tp,
