...
class Model(FModule):
    ...
    def forward(self, x, y, leads, quantile=0.75, visualize=False, matrix=None, contrastive_weight=5.0): # x: B x 12 x C
        ...
        total_lead_inds = list(range(12))
        for lead in total_lead_inds:
            ...
            # start extraction
            lead_feature = self.feature_extractors[lead](x[:, lead, :].view(batch_size, 1, -1))
            features += lead_feature * (1-missing_mask)
            # features += lead_feature
