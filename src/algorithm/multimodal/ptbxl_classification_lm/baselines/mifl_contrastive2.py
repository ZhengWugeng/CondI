...
class Server(BasicServer):
    ...
    def save_checkpoints(self):
        ...
        outdir = os.path.join(self.checkpoints_dir, 'MIFL', 'global-model')
        os.makedirs(outdir, exist_ok=True)
        torch.save(self.model.state_dict(), os.path.join(outdir, f'model.pt'))

    def load_checkpoints(self):
        if os.path.exists(os.path.join(self.checkpoints_dir, 'MIFL', 'global-model')):
    ...
    def aggregate(self, models: list, modalities_list: list):
        ...
        for m in range(self.n_leads):
            global_relation_embedders.append(fmodule._model_average([
                self.clients[k].local_model.relation_embedders[m] for k in self.selected_clients
            ])    )
        for k in range(n_models):    
            for m in range(self.n_leads):
...
class Client(BasicClient):
    ...
    @fmodule.with_multi_gpus
    def test(self, model, dataflag='train', quantile=0.75):
        """
