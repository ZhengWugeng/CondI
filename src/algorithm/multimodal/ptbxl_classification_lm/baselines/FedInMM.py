...
class Server(BasicServer):
    ...
    def aggregate(self, models: list, modalities_list: list):
        ...
        new_model.fmsm = fmodule._model_sum([
            model.fmsm * pk for model, pk in zip(models, p)
        ])
        new_model.cgfm = fmodule._model_sum([
            model.cgfm * pk for model, pk in zip(models, p)
        ])
        return new_model
...
class Client(BasicClient):
    ...
    def train(self, model):
        ...
        for iter in range(self.num_steps):
            ...
            # for n, p in model.named_parameters():
            #     if p.requires_grad:
            #         # print(n, p.clone().mean().item())
            #         print(n, p.grad.mean().item())
            #         break
