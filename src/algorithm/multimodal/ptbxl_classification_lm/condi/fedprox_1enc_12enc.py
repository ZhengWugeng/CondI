...
class Server(BasicServer):
    ...
    def save_checkpoints(self):
        ...
        outdir = os.path.join(self.checkpoints_dir, 'Original', 'global-model')
        os.makedirs(outdir, exist_ok=True)
        torch.save(self.model.state_dict(), os.path.join(outdir, f'model.pt'))

    def load_checkpoints(self):
        if os.path.exists(os.path.join(self.checkpoints_dir, 'Original', 'global-model')):
        ...
        for iter in range(self.num_steps):
            ...
            if self.fedmsplit_prox_lambda > 0.0:
                for m in self.modalities:
                    for parameter, agg_parameter in zip(model.pre_extractors[m].parameters(), self.agg_model.pre_extractors[m].parameters()):
                        regular_loss += torch.sum(torch.pow(parameter - agg_parameter, 2))
                    for parameter, agg_parameter in zip(model.sim_projectors[m].parameters(), self.agg_model.sim_projectors[m].parameters()):
                        regular_loss += torch.sum(torch.pow(parameter - agg_parameter, 2))
                for parameter, agg_parameter in zip(model.classifier.parameters(), self.agg_model.classifier.parameters()):
                    regular_loss += torch.sum(torch.pow(parameter - agg_parameter, 2))
                for parameter, agg_parameter in zip(model.feature_extractors[0].parameters(), self.agg_model.feature_extractors[0].parameters()):
                    regular_loss += torch.sum(torch.pow(parameter - agg_parameter, 2))
